...
// CASIndex is a per-tenant cas.Index backed by the fabriq_blob_cas table.
...
// casRow is the internal scan target for SELECT rows from fabriq_blob_cas.
type casRow struct {
...
func (c *CASIndex) Get(ctx context.Context, hash string) (*trovecas.Entry, error) {
	...
	err := c.run.TenantTxRaw(ctx, func(tx *pgdriver.PgTx) error {
		return tx.NewRaw(
			`SELECT hash, bucket, key, size, ref_count, pinned FROM fabriq_blob_cas WHERE hash = $1`,
			hash,
		).Scan(ctx, &rows)
	})
	if err != nil {
...
func (c *CASIndex) Put(ctx context.Context, e *trovecas.Entry) error {
	return c.run.TenantTxRaw(ctx, func(tx *pgdriver.PgTx) error {
		_, err := tx.NewRaw(
			`INSERT INTO fabriq_blob_cas (id, tenant_id, hash, bucket, key, size, ref_count, pinned)
			 ...
			 ON CONFLICT (tenant_id, hash) DO UPDATE
			   SET bucket    = EXCLUDED.bucket,
			       key       = EXCLUDED.key,
			       size      = EXCLUDED.size,
			       ref_count = fabriq_blob_cas.ref_count + 1`,
...
func (c *CASIndex) Delete(ctx context.Context, hash string) error {
	return c.run.TenantTxRaw(ctx, func(tx *pgdriver.PgTx) error {
		res, err := tx.NewRaw(
			`DELETE FROM fabriq_blob_cas WHERE hash = $1`, hash,
		).Exec(ctx)
		if err != nil {
			return err
		}
...
func (c *CASIndex) IncrementRef(ctx context.Context, hash string) error {
	return c.run.TenantTxRaw(ctx, func(tx *pgdriver.PgTx) error {
		res, err := tx.NewRaw(
			`UPDATE fabriq_blob_cas SET ref_count = ref_count + 1 WHERE hash = $1`, hash,
		).Exec(ctx)
		if err != nil {
			return err
		}
...
func (c *CASIndex) Pin(ctx context.Context, hash string) error {
	return c.run.TenantTxRaw(ctx, func(tx *pgdriver.PgTx) error {
		res, err := tx.NewRaw(
			`UPDATE fabriq_blob_cas SET pinned = true WHERE hash = $1`, hash,
		).Exec(ctx)
		if err != nil {
			return err
		}
...
func (c *CASIndex) Unpin(ctx context.Context, hash string) error {
	return c.run.TenantTxRaw(ctx, func(tx *pgdriver.PgTx) error {
		res, err := tx.NewRaw(
			`UPDATE fabriq_blob_cas SET pinned = false WHERE hash = $1`, hash,
		).Exec(ctx)
		if err != nil {
			return err
		}
