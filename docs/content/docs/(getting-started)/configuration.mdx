...
Fabriq takes its configuration two ways: library code passes a `fabriq.Config` value (plus functional `Option`s) to `fabriq.Open`, and the `fabriq` binary reads the same shape from environment variables. This page is task-oriented — the exhaustive field-by-field reference lives at [Reference: Configuration](/docs/reference/configuration).
...
<Tabs items={['Library (Go)', 'Binary (env)']}>
...
The binary builds the equivalent `Config` from these variables in its `setup` path. Subscription-plane knobs and projection toggles are not env-driven in the binary today — tune them in library wiring when embedding the facade.
...
The `fabriq` binary reads these. `--dsn` is a global flag that overrides `FABRIQ_POSTGRES_DSN` for ad-hoc operator commands.
