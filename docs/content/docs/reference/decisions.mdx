...
<Callout type="info">
  The tenant→shard routing layer, multi-shard `Open`, and the per-shard outbox relay are **implemented**. A catalog-backed placement directory (with controlled tenant moves), online rebalancing, and a shard-aware migrate CLI are **not yet** — see [Sharding](/docs/sharding).
</Callout>
...
## Optional remote protocol — fabriq-as-a-server

<Callout type="info">
  The write, relational-read and subscribe planes plus edge authentication (mTLS
  + bearer, tenant stamping) are **implemented** and round-trip-tested over real
  gRPC. The maintained live-query, blob streaming and interactive-transaction
  planes are **not yet**; raw-SQL `Query` is held by policy — see [Remote
  protocol](/docs/remote-protocol).
</Callout>

Fabriq is an embedded library first — each service owns its datastore pools and holds the facade in-process. The remote protocol adds fabriq-as-a-server as an *optional topology over the same `core/query.Fabric` interface*, so call sites are identical whether the facade is embedded or remote. The decision is faithful to the layering above, not a break from it: Postgres stays the single linearizable anchor and the server tier is a transport in front of it, not a second writer — there are no distributed transactions (`ExecBatch` is one single-node transaction; interactive `Tx`, when built, is pinned to one replica).

The transport is **gRPC over HTTP/2**, chosen because only backends connect — no browser removes the one real reason to avoid it: native bidirectional streaming for live deltas and file bytes, one multiplexed channel per client (which *is* the connection-consolidation goal), and deadline/cancellation propagation that maps onto fabriq's ctx-carried tenant. The envelope is **protobuf framing with opaque JSON/bytes payload bodies** — the registry, not the wire schema, stays the authority on payload shape, so dynamic entities and map-native writes cross unchanged. Identity is **authenticated at the edge** from the verified credential and stamped as the tenant, never trusted from a request field — a strict upgrade over the in-process trust-the-context model.

The gRPC binding lives in its **own Go module** (`remote/grpc`) so a service that only embeds the library never pulls `google.golang.org/grpc` into its dependency tree — the same dependency-fencing discipline that keeps the byte store out of the core.

<Cards>
  <Card title="Tenancy" href="/docs/tenancy">How the tenancy layers and the telemetry exception work in practice.</Card>
  <Card title="Sharding" href="/docs/sharding">The tenant-routed, multi-shard source of truth.</Card>
  <Card title="Remote protocol" href="/docs/remote-protocol">The optional fabriq-as-a-server topology and its gRPC binding.</Card>
