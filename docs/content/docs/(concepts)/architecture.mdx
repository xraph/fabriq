...
**`domain/` is the only TWINOS-aware package.** It registers the entity pack (site, asset, tag, page) as `registry.EntitySpec` values. Swap it for your own pack against a fresh `registry.New()` to model a different domain; the kernel and adapters do not change.
