...
The TWINOS pack registers four entities. Here is the aggregate `asset` and the document `page`, verbatim from `domain/register.go`:
...
<Callout type="info">
`RegisterAll` registers the TWINOS pack (site, asset, tag, page). To model your own domain, build the same shape of `EntitySpec` set against a fresh `registry.New()`. The kernel and adapters are unchanged.
</Callout>
