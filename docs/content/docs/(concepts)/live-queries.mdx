...
- **Maintained** (default) — the exact ordered window described above.
...
A **reconcile backstop** (`f.ReconcileLiveQueries(ctx)`, run on a low cadence by the worker) re-checks each maintained subscription against Postgres truth and re-snapshots any that drifted (emitting `OpReset`), so transient predicate-evaluation divergence self-heals.

## Roadmap

Built today: the Maintained and Streamed engines, the predicate-index dispatcher, and the reconcile backstop, all single-node. Still planned:

- **Templated sharing** — identical saved-view queries share one compiled matcher and buffer (ref-counted), instead of one per subscriber.
...
- **Sharding & gateway tier** — data-partitioned matcher shards fed by partitioned Redis streams, a durable subscription registry, and failover via re-snapshot — the step from one process to many.
