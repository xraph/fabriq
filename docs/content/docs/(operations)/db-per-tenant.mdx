...
- The version gate (fleet upgrades, above) is **fail-closed**: if a
  ...
  self-healing — it clears once the replica catches up or the primary
  recovers — never a silent correctness hazard.
...
| Replica reports a stale (behind-floor) version | Router fails closed (`CodeUnavailable`) for that read — temporary and self-healing, never a route to a schema-behind database |
