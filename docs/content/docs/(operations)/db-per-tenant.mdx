...
- The version gate (fleet upgrades, above) is **fail-closed**: if a
  ...
  self-healing — it clears within one cache TTL once the replica catches up
  or the primary recovers (a version-gated read taken during an outage is
  cached like any other routing answer) — never a silent correctness hazard.
...
[analytics sink](/docs/analytics-sink) reads the same shared event stream
...
| Replica reports a stale (behind-floor) version | Router fails closed (`CodeUnavailable`) for that read — self-heals within one cache TTL, never a route to a schema-behind database |
...
| Cross-tenant reporting | [analytics sink](/docs/analytics-sink) (opt-in, mode-agnostic) | [analytics sink](/docs/analytics-sink) (opt-in, mode-agnostic) |
