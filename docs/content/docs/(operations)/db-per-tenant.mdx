...
- **Clusters** — ordinary Postgres servers named in `clusterDsns`. Spread
  ...
  count. Add clusters before a cluster's database count gets silly.
...
reconciler, live queries and document history archiving are not yet
routed in catalog mode (they return typed errors or are documented gaps).
