...
<Callout type="info">
Planned, not yet built: a `fabriq_analytics_lag_seconds` gauge (needs a
database-side poller comparing applied watermarks to stream position) and a
`fabriq_analytics_backfill_rows_total` counter (backfill runs outside the
worker's metrics loop today). See [Analytics sink](/docs/analytics-sink).
</Callout>
