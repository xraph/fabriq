...
// no theme detection, no client JS. Every primitive comes from ./diagram-kit;
// this file holds the two layouts and nothing else.
...
export function ArchitectureDiagram() {
  ...
  return (
    <Figure
      viewBox="0 0 960 560"
      title="The layered fabriq architecture"
      desc="Application code and AI agents hold one facade over the engine-agnostic core kernel — registry, command, event, projection, subscribe, query, cache, blob, and agent — which reaches the capability ports, the adapter dialects (including trove for blob CAS and grove-kv for the cache), and the backing engines, with Postgres the source of truth."
    >
      {({ Arrow }) => (
        <>
          {/* Application + agents */}
      ...
      {({ Arrow }) => (
        <>
          ...
          <text
            ...
            fill={INK_MUTED}
          >
            Reads bypass the stream — application → capability ports
            (read-through cache) → each engine directly
          </text>
        </>
      )}
    </Figure>
